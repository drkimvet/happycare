...
class PublicCardInvariants(unittest.TestCase):
    ...
    def test_verification_log_flags_outdated_and_unit_traps(self):
        ...
        self.assertIn("ehrlichiosis", VERIF.lower())
        self.assertIn("4dx / snap antibody is exposure", VERIF.lower())
        self.assertIn("rmsf before the titer", VERIF.lower())
        self.assertIn("chloramphenicol is not the plan", VERIF.lower())
        self.assertIn("lyme borreliosis", VERIF.lower())
        ...
        self.assertIn("feline cryptococcosis", VERIF.lower())
        self.assertIn("narrow-based budding", VERIF.lower())
        self.assertIn("fluconazole 10", VERIF.lower())
        self.assertIn("do not use flucytosine in dogs", VERIF.lower())
        self.assertIn("canine histoplasmosis", VERIF.lower())
        self.assertIn("inside macrophages", VERIF.lower())
        self.assertIn("itraconazole 10", VERIF.lower())
        self.assertIn("warn the lab", VERIF.lower())
        self.assertIn("canine coccidioidomycosis", VERIF.lower())
        self.assertIn("spherules", VERIF.lower())
    ...
    def test_vetspire_macros_are_paste_ready_and_not_a_login(self):
        ...
        self.assertIn("`ddx-ehr`", MACRO)
        self.assertIn("`dc-ehr`", MACRO)
        self.assertIn("do not catch this from the dog", MACRO.lower())
        self.assertIn("well 4dx as active infection", MACRO.lower())
        self.assertIn("`ddx-lyme`", MACRO)
        ...
        self.assertIn("`ddx-crypto`", MACRO)
        self.assertIn("`dc-crypto`", MACRO)
        self.assertIn("no flucytosine in dogs", MACRO.lower())
        self.assertIn("roman-nose cat home as just a cold", MACRO.lower())
        self.assertIn("`ddx-histo`", MACRO)
        self.assertIn("`dc-histo`", MACRO)
        self.assertIn("tiny yeasts inside macrophages", MACRO.lower())
        self.assertIn("warn the lab", MACRO.lower())
        self.assertIn("`ddx-cocci`", MACRO)
