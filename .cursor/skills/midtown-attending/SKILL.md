...
8. **Compact night format.** Midtown answers stay short. Korean is allowed. No owner names, phones, or chart IDs in git or in reusable notes. Vetspire DDX/discharge phrases live in `clinical/vetspire_macros.md`. Do not login to Vetspire. The attending pastes them under More → Macros and inserts with `\`. Insert `dc-gi` only if THIS patient has a GI localization; insert `dc-toxin` only if THIS patient has a toxin clock.
